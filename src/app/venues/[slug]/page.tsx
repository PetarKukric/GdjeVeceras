            ...
            <div id="informacije" className="bg-card border border-white/5 rounded-[2.5rem] p-8 shadow-2xl space-y-8 relative overflow-hidden group">
               ...
               <div className="space-y-6">
                  <div className="flex items-center justify-between">
                     ...
                     {venueStatus && (
                       <div className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${venueStatus.status === 'OPEN' ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-red-500/10 text-red-500 border border-red-500/20'}`}>
                          {venueStatus.label}
                       </div>
                     )}
                  </div>
                  ...
                  {venueStatus && (
                    <div className="flex gap-4">
                       ...
                       <div>
                          <p className="text-[9px] font-black text-muted uppercase tracking-widest mb-0.5">{venueStatus.label === 'OTVORENO' ? 'Radi do' : 'Otvara se u'}</p>
                  ...
                  {['WEEKDAYS', 'FRIDAY', 'SATURDAY', 'SUNDAY'].map(group => {
                    ...
                    return (
                      <div key={group} className="flex justify-between items-center text-xs">
                        <span className="text-muted font-bold uppercase tracking-widest text-[9px]">{label}</span>
                        <span className="text-white font-black">
                          {hours?.isClosed ? (
                            <span className="text-red-500/80 uppercase tracking-widest text-[9px]">Zatvoreno</span>
                          ) : (
            ...
            <div className="bg-card border border-white/5 rounded-[2.5rem] p-8 shadow-2xl space-y-8 relative overflow-hidden group text-left">
               ...
               <div className="space-y-6">
                  ...
                  <a 
                    ...
                    target="_blank"
                    className="w-full py-4 bg-surface border border-white/5 text-white font-black rounded-2xl hover:bg-white/5 transition-all flex items-center justify-center gap-3 uppercase tracking-[0.2em] text-[9px] shadow-lg group/nav"
                  >
            ...
            {venue.tags && venue.tags.length > 0 && (
              <div className="bg-card border border-white/5 rounded-[2.5rem] p-8 shadow-2xl space-y-8 relative overflow-hidden group text-left">
                 ...
                 <div className="flex flex-wrap gap-2">
                    {venue.tags.map((tag: any) => (
                      <div key={tag.id} className="px-3 py-1.5 bg-surface border border-white/5 rounded-lg text-[8px] font-black text-muted uppercase tracking-widest hover:border-primary/50 transition-all cursor-default">
                         {tag.name}
                      </div>
                    ))}
                 </div>
