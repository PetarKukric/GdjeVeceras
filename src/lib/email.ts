...
const WRAPPER = (inner: string) => `
  <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; background: #18181D; color: #FFFFFF; border-radius: 16px;">
    <h2 style="color: #FF006E; margin: 0 0 16px; text-transform: uppercase;">Gdje Večeras</h2>
    ${inner}
...
export async function sendVerificationEmail(email: string, token: string) {
  ...
  return sendMail(
    ...
    WRAPPER(`
      ...
      <a href="${verificationLink}" style="display: inline-block; margin: 16px 0; padding: 14px 28px; background: #FF006E; color: #FFFFFF; text-decoration: none; border-radius: 10px; font-weight: bold; text-transform: uppercase; font-size: 13px; letter-spacing: 1px;">Potvrdi email</a>
...
export async function sendAdminPasswordEmail(email: string, password: string) {
  return sendMail(
    ...
    WRAPPER(`
      ...
      <div style="margin: 16px 0; padding: 18px; background: #1A1825; border: 1px dashed #FF006E; border-radius: 10px; text-align: center;">
...
export async function sendPasswordResetEmail(email: string, token: string) {
  ...
  return sendMail(
    ...
    WRAPPER(`
      ...
      <a href="${resetLink}" style="display: inline-block; margin: 16px 0; padding: 14px 28px; background: #FF006E; color: #FFFFFF; text-decoration: none; border-radius: 10px; font-weight: bold; text-transform: uppercase; font-size: 13px; letter-spacing: 1px;">Postavi novu lozinku</a>
